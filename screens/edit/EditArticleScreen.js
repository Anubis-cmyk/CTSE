...
const EditArticleScreen = ({route}) => {
    ...
    if (route.params?.item) {
        const { id,name, shortInfo, longInfo,tags,imageUrl,voteCount ,votedBy} = route.params.item;
        setImage(imageUrl);
        setName(name);
        setId(id);
        setShortInfo(shortInfo);
        setLongInfo(longInfo);
        setTagsList(tags);
    ...
    const handleSubmit = async () => {
        ...
        const articleRef = database.collection('articles').doc(id);
        await articleRef.set({
            ...
            setTagsList([]); 
            setVotedBy([]);
            alert('Article edit successfully!');
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : ''}>
        ...
        <View style={styles.container}>
         <View style={styles.header}>
            ...
            <Animatable.Text animation="fadeInDown" duration={1000} style={styles.title}>Edit article details</Animatable.Text>
          </View>
